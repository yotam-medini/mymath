...
%%%%%
\begin{excopy}
Let $G$ be agroup. A \textbf{commutator} in $G$ is
...
it was shown that when $H$ is contained in the normalizer of $K$, then
...
%%%%%
\begin{excopy}
Let $G$ be a finite groupopeon a finite set $S$.
 \begin{itemize}
  \item[(a)]
...
Since any $p$-subgroup is continaed in a $p$-Sylow subgroup,
...
\begin{itemize}
 \item[(a)] (Following argument in the proof of Theorem~6.4).
     ...
     we have \(P'P\) is a sybgroup of \(N(P)\) and $P$ is normal in it.
     Now
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\textbf{Explicit determiniation of groups}
...
The groups with prime order are cyclic. For otehr case of a group $G$:
\begin{itemize}
...
\begin{itemize}
 \item[(a)]
   ...
   of order \(2n\), generated bt two elements \(\sigma\), \(\tau\)
...
\iffalse
\begin{itemize}
  \item[(a)] Rotation and mirroring. Consider the subgruop of \(S_n\)
     where
   \begin{equation*}
     \sigma(i) =
       \left\{
...
begin the only element of orer $2$ in $H$.
...
Now since the ordes of $j$ and $k$ are also $4$,
...
Let $H$ be a subgroup of $G$. If \([G:h]=1\) then \(H \subnormal G\).
\end{llem}
\begin{proof}
Let \(g \in G\setminus \) then
\begin{equation*}
