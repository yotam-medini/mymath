...
Let \(T=E(\omega)\in\scrB(H)\). So \(E_{x,y}(\omega) = \lrangle{Tx,y}\).
We need to show
...
The inequality trivially holds if \(Tx = 0\).
So we may assume \(Tx \neq 0\) and clearly \(x\neq 0\).
Let \(y = aTx + y_\perp\) be such that \(a\in\C\) and
\(\lrangle{Tx, y_\perp} = 0\).
