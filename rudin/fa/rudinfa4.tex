...
Hence \(\dim V = \dim V^{**}\).
Now in finite-dimensional vector space, a ???  sub-space
...
eigenvalue are \(\sin n\pi x\), and that each eigenspace is one-dimensional.
\emph{Hint:} If
...
&= \int_s^1 f(t)\,dt - \int_0^1 t\cdot f(t)\,dt
\end{align*}
Hence
\begin{equation*}
(Tf)''(s) = -f(s).
...
prove that \(T \in \scrB(L^2)\) and that $T$ is not compact.
 (The fact that \(\|T\| \leq 2\) is a special cas
