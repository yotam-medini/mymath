...
For example \cite{Kelley2017general} Chapter 5, 13~Theorem \textsc{Tychonoff}.
...
Let \(x \in F_j\), its inverse imaage \(M = {\pi_j}^{-1}(x) \subset A\)
contains the point \(p\in M\)
such that \(P_j = x \in A_j\) and \(P_i = i\) for \(i \neq j\).
Thus \(M\neq \emptyset\) and so any inverse imaage of non empty
