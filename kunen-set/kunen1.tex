...
Let \(\emptyset\neq X \subseteq F(\alpha, \beta)\).
Denote the constant zero function \(\zeta\in F(\alpha, \beta)\), \(\zeta(x)=0\)
...
let \(y = \min(Y)\). Now pick some \(f\in F(\alpha, \beta)\)
such that \(f(\xi)=y\). Note that \(f(x)=0\) for all \(x>\xi\).
Clearly $f$ is mininal in $X$.
Thus \(\triangleleft\) well orders \(F(\alpha, \beta)\)
\unfinished
