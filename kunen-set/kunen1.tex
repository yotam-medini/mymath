...
Clearly \(1= \{\empty\}\) is the only singleton transitive set.

If $T$ is non-singleton, let \(S = T \setminus \{\emptyset\} \neq \emptyset\).
Since $S$ is not transitive, let \(u \in S\) be such that \(u \subsetneq S\).
Since \(u\subset T\), we have \(\emptyset \in u\)
...
Let \(S\subset z\) some non-empty set. By Axiom of Foundation,
...
If $z$ is an ordinal, then by definition it is transitive.
Let \(s\in z\) and by negation is not transitive.
Thus we have \(e\in s \land e \subsetneq s\).
\unfinished
