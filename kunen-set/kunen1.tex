...
% Clearly \(X_n^n=\kappa^n=\kappa\)
% and so \(\cup_{n < \omega} X_n = \kappa = \alpha\).

% By negation, we may assume that  \(\kappa\) is the minimal ordinal
% for which the Milner-Rado claim fails.
...
Implicitlt, \(\alpha\) is an ordinal.
...
Denote
\begin{equation*}
\otimes(j) = \kappa^{k_0} \times \kappa^{k_1} \times \cdots \times \kappa^{k_{j-1}}.
\end{equation*}
Consider the isomorphism
\begin{equation*}
% \kappa^{k_0} \times \kappa^{k_1} \times \cdots \times \kappa^{k_{c-1}}
\otimes(c)
\quad\leftrightarrow\quad
% \kappa^{k_0+k_1+ \cdots + k_n}
\kappa^{k_0+k_1+ \cdots + k_{c-1}}
= \kappa^{\sum_{n<c} k_n}
\end{equation*}
provided by the lexical ordering (see Definition I.8.4).
Since
\begin{equation*}
...
\begin{equation*}
% f_0 : \kappa^{k_0} \times \kappa^{k_1} \times  \dots \kappa^{k_{c-1}}
f_c : \otimes(c)
\to \alpha = \cup_{n<c} X_n.
\end{equation*}

More generally, we define by (decreasing) induction
on \(0<i<c\) an increasing function
\begin{equation*}
f_i : \otimes(i) \to \cup_{n<c-i} X_n.
\end{equation*}
If \(f_i\) is defined for \(i<c\), then for any
\(\beta \in \otimes(i-1)\)
we can find \(\gamma_\beta < \kappa^{k_i}\) such that
\(f_i(\beta, \gamma_\beta) \notin X_i\).
Otherwise, \(X_{c-i}\) contains the increasing sequence
\(\{f_i(\beta,\gamma): \gamma < \kappa^{k_{c-i}}\}\)
and we have \(\type(X_{c-i}) \geq \kappa^{k_{c-i}}\),
a contradiction.
Hence we define for any
\(\beta \in \otimes(c-i-1)\), picking the minimal such \(\gamma_\beta)\)
\begin{equation*}
f_{i-1}(\beta) = f_i(\beta, \gamma_\beta)
 \in \left(\cup_{n < i} X_n\right) \setminus X_i
 = \cup_{n < i-1} X_n\,.
\end{equation*}
Not that the \(f_i\) is increasing in \(\beta\)
since if \(\beta_x < \beta_y\) then
\begin{equation*}
f_i(\beta_x, \gamma_{\beta_x}) = f_i(\beta_y, \gamma_{\beta_y})
\end{equation*}
even if \(\gamma_{\beta_x} > \gamma_{\beta_y}\)
since the \(\gamma\)s appear after the \(\beta\)s in lexicographic order.

Finally, \(f_1: \kappa^{k_0} \to X_0\)
