...
\iffalse
If \alpha is a limit ordinal and \omega \beta 1⁢n1 + … + \omega \beta k⁢nk its Cantor Normal Form, then from the previous relation we deduce \gamma⁢(\alpha)=\gamma⁢(\omega \beta 1) + \omega \beta 1 \cdot 2⁢(n1-1) + \omega \beta 1 +  \beta 2 \cdot n2 + … + \omega \beta 1 +  \beta k \cdot nk. In particular, \gamma⁢(\alpha)>\alpha if k \geq 2 or n1 \geq 2. For all \alpha \geq 1, \log\omega⁡(\omega\alpha) + 1=\alpha + 1 so the previous paragraph also gives \gamma⁢(\omega\alpha + 1)=\gamma⁢(\omega\alpha + \omega\alpha + 1)=\gamma⁢(\omega\alpha) + \omega\alpha \cdot 2 + 1. In particular, \gamma⁢(\omega\alpha + 1) \geq \omega\alpha \cdot 2 + 1>\omega\alpha + 1. Moreover, by induction on n<\omega, we can show that \gamma⁢(\omegan + 1)=\omega2⁢n + 1 and so \gamma⁢(\omega\omega)=sup1\leq n<\omega⁡\gamma⁢(\omegan)=\omega\omega.
...
For any 1\leq n<\omega and \alpha \geq 1, if 1\leq  \beta <\omega\alpha then \log\omega⁡( \beta )<\alpha=\log\omega⁡(\omega\alpha \cdot n). Hence the relation of the previous paragraph gives \gamma⁢(\omega\omega\alpha \cdot n +  \beta )=\gamma⁢(\omega\omega\alpha \cdot n) + \omega\omega\alpha \cdot 2⁢n +  \beta . Then by continuity of \gamma and of the sum/exponentiation in the second variable, we can consider the limit  \beta →\omega\alpha. This gives \gamma⁢(\omega\omega\alpha \cdot (n + 1))=\gamma⁢(\omega\omega\alpha \cdot n) + \omega\omega\alpha \cdot (2⁢n + 1). By induction, we deduce on 2\leq n<\omega we deduce \gamma⁢(\omega\omega\alpha \cdot n)=\gamma⁢(\omega\omega\alpha) + \omega\omega\alpha \cdot (2⁢n-1) and in particular \forall2\leq n<\omega,\gamma⁢(\omega\omega\alpha \cdot n)>\omega\omega\alpha \cdot n. We can also consider the limit n→\omega and we obtain \gamma⁢(\omega\omega\alpha + 1)=\gamma⁢(\omega\omega\alpha) + \omega\omega\alpha + 1.
